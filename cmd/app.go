...
func (a *App) Run(ctx context.Context) error {
	...
	group.Go(func() error {
		return client.Run(ctx, func(ctx context.Context) error {
			...
			if a.cfg.Password != "" {
				flow = auth.NewFlow(auth.Constant(a.cfg.Phone, a.cfg.Password, auth.CodeAuthenticatorFunc(getCode)), auth.SendCodeOptions{})
			} else {
			...
			err, tgMessages := msg.GetLastNMessages(ctx, api, a.cfg.Chats, a.cfg.N)

			if err != nil {
				log.Println(xerrors.Errorf("get messages: %w", err))
				err, tgMessages = retry(ctx, api, a.cfg.Chats, a.cfg.N)
			}
...
func retry(ctx context.Context, api *tg.Client, chats []int, n int) (error, []tg.Message) {
	var err error
